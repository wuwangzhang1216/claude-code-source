...
> **跟随官方更新，保持最新版本。** 本仓库跟随 Claude Code 官方发布节奏，始终与最新版本保持同步。官方每次发布新版本后，我们都会针对其 changelog 中面向用户、可落地的部分进行对应更新。每个版本的具体变更记录（包括本仓库已应用的改动和仅存在于官方的改动）请见 [`CHANGELOG.md`](CHANGELOG.md)。
...
本仓库支持从源码本地启动 Claude Code（功能有所缩减）。已验证可成功运行，版本号 `2.1.89`。
...
| **认证方式** | Claude Pro/Max/Team 订阅（OAuth）、API Key（`sk-ant-xxx`）、ChatGPT 订阅、或 OpenAI API Key |
...
<details>
<summary><strong>备选方案：使用 ChatGPT 订阅（GPT-5.4）</strong></summary>

如果你有 ChatGPT Plus/Pro/Team 订阅，可以直接使用 GPT-5.4 作为 Agent 模型，无需 API Key：

```bash
# 1. 使用 ChatGPT 账号登录（会打开浏览器）
...
<p align="center">
  <img src="assets/chatGPT-sub.png" alt="通过 ChatGPT 订阅运行 GPT-5.4" width="100%">
  <br>
  <em>通过 ChatGPT 订阅使用 GPT-5.4 作为 Agent 模型</em>
</p>

可用的 GPT 模型别名：`gpt5.4`、`gpt5.4-mini`、`gpt5.4-nano`

也可以使用完整格式：`--model chatgpt:gpt-5.4`

</details>

<details>
<summary><strong>备选方案：使用 OpenAI API Key</strong></summary>
...
| `start.sh` | macOS/Linux 启动脚本，支持 OAuth 登录（`./start.sh login`）、多种凭证检测（Keychain/文件/环境变量）、第三方代理自动检测、Bun 编译缓存自动清理 |
...
| 只能在一个目录启动 / 移动源码后卡死 | Bun 按绝对路径缓存编译结果。`start.sh` 会自动清理缓存。手动启动时需先清理：`rm -rf ~/Library/Caches/bun`（macOS）或 `rm -rf ~/.cache/bun`（Linux） |
